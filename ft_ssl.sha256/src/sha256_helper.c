...
/*   Updated: 2018/07/13 11:42:05 by egoodale         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../ft_ssl.h"

#define guide_sha256_add_length(context, length)               \
  (addTemp = (context)->len_low, (context)->corrupted = \
    (((context)->len_low += (length)) < addTemp) &&     \
    (++(context)->len_high == 0) ? EINPUTOOLONG :    \
                                      (context)->corrupted )
